...
Respond with ONLY "YES" or "NO"."""
    ...
    async def validate_media_caption(self, captions: list[str]) -> tuple[bool, str]:
        """
        Validates if CLIP-generated captions describe agriculture-related content.
        Uses Gemini to analyze the text descriptions.
        
        Args:
            captions: List of text descriptions from CLIP/BLIP model
        ...
        if not captions:
            return False, "Could not analyze the media content"
        
        # Combine all captions for analysis
        combined_captions = "\n".join([f"- {cap}" for cap in captions])
        
        try:
            prompt = f"""You are a STRICT content moderator for an Indian farmer community chat.
Analyze these image/video descriptions generated by an AI vision model:

{combined_captions}

Based on these descriptions, determine if the media content is AGRICULTURE-RELATED.

ALLOWED content (respond "YES"):
- Crops, farms, fields, gardens, plantations
- Farmers, farm workers, rural workers
- Tractors, farm equipment, agricultural machinery
- Farm animals (cows, goats, chickens, buffaloes, etc.)
- Plants, seeds, soil, fertilizers, irrigation
- Rural landscapes, villages, farmhouses
- Fruits, vegetables, grains, harvest
- Weather conditions affecting farming
- Agricultural markets, mandis

BLOCKED content (respond "NO"):
- Selfies not in farm context
- City buildings, urban scenes
- Cars, bikes, vehicles (non-farm)
- Random people, parties, events
- Memes, screenshots, text images
- Inappropriate or unrelated content

Be STRICT. If the descriptions don't clearly indicate agriculture/farming content, block it.
Respond with ONLY "YES" or "NO"."""
            ...
            result = response.text.strip().upper()
            print(f"Caption Validation Result: {result} for captions: {captions}")
            
            if "YES" in result:
                return True, "Content approved"
            else:
                return False, "Only agriculture-related images and videos are allowed in this community"
                
        except Exception as e:
            print(f"Caption validation error: {e}")
            return False, "Could not validate media content. Please try again."
